...
public class Home {
    ...
    @FXML
    private void searchButtonAction() throws IOException {
        ...
        searchbar.textProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue.length() > 0) {
                String capitalizedText = newValue.substring(0, 1).toUpperCase() + newValue.substring(1);
                searchbar.setText(capitalizedText);
            }
        });

        List<Movies> ml = dm.getAllMoviesForGenre(dm.getGenreIdByGenreName(searchbar.getText()));
        ...
        for (int i = 0; i < ml.size(); i++){
            ...
            List<Genres> gl = dm.getAllGenresForMovie(i);
    ...
    public void initialize() {
        ...
        List<Movies> ml = dm.getAllMovies();
